...
export function SelectionCatalogDriveSystemDictPage() {
  ...
  const [drvCategories, setDrvCategories] = useState<SelDictCategoryRead[]>([]);
  ...
  const loadCategories = useCallback(
    async (selectCode?: string) => {
      ...
      try {
        ...
        const sorted = sortDrvCategories(rows.filter((c) => isDrvCategory(c.code)));
        setDrvCategories(sorted);
  ...
  const refreshBundlePreview = useCallback(async () => {
    ...
    try {
      const bundle = await fetchDriveSystemDetailDictBundle();
  ...
  const catLabel = drvCategories.find((c) => c.code === categoryCode)?.label ?? categoryCode;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-slate-800">驱动系统字典</h1>
        <p className="mt-1 text-sm text-slate-600">
          数据来自《驱动系统》截图/Excel；分类 <Typography.Text code>code</Typography.Text> 以{" "}
          <Typography.Text code>hrspec_drv_</Typography.Text> 开头。型号与 BOM 分多条分类存储，级联由业务页实现。
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <label className="block min-w-[240px] text-sm">
          <span className="text-slate-600">字典分类（驱动系统）</span>
          <Select
            className="mt-1 w-full min-w-[320px]"
            loading={loadingCats}
            value={categoryCode || undefined}
            options={drvCategories.map((c) => ({
              value: c.code,
              label: `${c.label} (${c.code})`,
            }))}
            onChange={(v) => setCategoryCode(v)}
            notFoundContent={loadingCats ? "加载中…" : "暂无 hrspec_drv_ 分类，请检查后端迁移"}
          />
        </label>
        <DictCategoryPrefixToolbar
          codePrefix={DRV_PREFIX}
          categories={drvCategories}
        ...
        <div className="text-xs text-slate-500">
          选项包预览（<Typography.Text code>GET …/drive-system-detail-options</Typography.Text>
          ）：{" "}
          {bundleLoading ? (
            "加载中…"
