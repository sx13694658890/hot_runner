...
export function SelectionCatalogManifoldDictPage() {
  ...
  const [mfldCategories, setMfldCategories] = useState<SelDictCategoryRead[]>([]);
  ...
  const loadCategories = useCallback(
    async (selectCode?: string) => {
      ...
      try {
        ...
        const sorted = sortMfldCategories(rows.filter((c) => isMfldCategory(c.code)));
        setMfldCategories(sorted);
  ...
  const refreshBundlePreview = useCallback(async () => {
    ...
    try {
      const bundle = await fetchManifoldDetailDictBundle();
  ...
  const catLabel = mfldCategories.find((c) => c.code === categoryCode)?.label ?? categoryCode;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-slate-800">分流板大类字典</h1>
        <p className="mt-1 text-sm text-slate-600">
          数据来自《热流道系统》分流板大类截图/Excel，分类 <Typography.Text code>code</Typography.Text> 以{" "}
          <Typography.Text code>hrspec_mfld_</Typography.Text> 开头；与扁平行热流道规格字段（
          <Typography.Text code>hrspec_*</Typography.Text> 无 <Typography.Text code>mfld</Typography.Text>
          ）独立。维护方式与「选型字典」相同，仅列出本前缀下的分类。
        </p>
      </div>
      ...
      <Alert
        ...
        message={
          <span>
            <strong>分流板材质</strong>仍使用热流道规格字典包中的{" "}
            <Typography.Text code>hrspec_manifold_material</Typography.Text>，请至{" "}
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <label className="block min-w-[240px] text-sm">
          <span className="text-slate-600">字典分类（分流板大类）</span>
          <Select
            className="mt-1 w-full min-w-[280px]"
            loading={loadingCats}
            value={categoryCode || undefined}
            options={mfldCategories.map((c) => ({
              value: c.code,
              label: `${c.label} (${c.code})`,
            }))}
            onChange={(v) => setCategoryCode(v)}
            notFoundContent={loadingCats ? "加载中…" : "暂无 hrspec_mfld_ 分类，请检查后端种子"}
          />
        </label>
        <DictCategoryPrefixToolbar
          codePrefix={MFLD_PREFIX}
          categories={mfldCategories}
        ...
        <div className="text-xs text-slate-500">
          选项包预览（<Typography.Text code>GET …/manifold-detail-options</Typography.Text>
          ）：{" "}
          {bundleLoading ? (
            "加载中…"
